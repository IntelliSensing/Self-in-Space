"""SIS-Bench dataset adapter for baseline and LoRA evaluation."""
import json
...
class SISBenchDataset(Dataset):
    """
    Dataset for SIS-Bench evaluation (baseline, no motion encoder).
    ...
    def _validate_frames(self):
        ...
        for item in self.data:
            ...
            if video_input.is_dir():
                frame_count = len([
                    path for path in video_input.iterdir()
    ...
    def _load_frames(self, item: dict):
        """Load and sample frames from a video file or frame directory."""
        video_input = self._resolve_video_input(item)
        if video_input is None:
        ...
        if video_input.is_file():
            return self._load_frames_from_video(video_input)
    ...
    def __getitem__(self, idx):
        ...
        pil_frames = self._load_frames(item)
        ...
        # Build Qwen3-VL-compatible chat messages
