...
from uav.train.trainer import replace_qwen2_vl_attention_class
...
from uav.data.data_processor import make_supervised_data_module
from uav.train.argument import (
    ModelArguments,
    DataArguments,
    TrainingArguments,
)
...
def build_callbacks(training_args):
    ...
    if not env_flag("SWANLAB_ENABLED", default=False):
        rank0_print("SwanLab logging disabled (set SWANLAB_ENABLED=1 to enable).")
        return callbacks
    ...
    try:
        callbacks.append(
            SwanLabCallback(
                project=os.getenv("SWANLAB_PROJECT", "uav-qwen3vl-finetune"),
    ...
    rank0_print(f"SwanLab logging enabled (mode={swanlab_mode}).")
    return callbacks
...
def train(attn_implementation="flash_attention_2"):
    ...
    # DeepSpeed/NCCL expects each process to bind its logical local rank
    # before distributed groups are created.
    if local_rank is not None and local_rank >= 0 and torch.cuda.is_available():
        torch.cuda.set_device(local_rank)
        print(
            f"[local_rank={local_rank}] bound to cuda:{torch.cuda.current_device()} "
            f"(visible={os.getenv('CUDA_VISIBLE_DEVICES', 'all')})"
        )

    model_family = resolve_model_family(model_args.model_name_or_path)
    rank0_print(f"Resolved model family: {model_family}")

    if model_family == "qwen3vl_moe":
