...
def print_trainable_parameters(self) -> None:
    ...
    Works with both old (embed_tokens/layers on self) and new (language_model sub-module) layouts.
    """
    # Resolve the text model: new layout has language_model, old has embed_tokens directly
    text_model = getattr(self, "language_model", self)

    # Check embed_tokens
    is_embed_trainable = any(
        param.requires_grad for param in text_model.embed_tokens.parameters()
    )
...
def _is_projector_param(name):
    """Check if parameter belongs to connector (uses mm_projector_lr)."""
    return "connector." in name


def _is_vision_tower_param(name):
    """Check if parameter belongs to vision tower."""
...
def create_optimizer(self):
    ...
    if self.optimizer is None:
        ...
        if self.args.mm_projector_lr is not None and self.args.mm_projector_lr != 0:
            # Use substring matching (robust to PEFT's base_model.model. prefix)
            projector_parameters = [
                n for n, _ in opt_model.named_parameters() if _is_projector_param(n)
            ]
            if self.args.vision_tower_lr is not None and self.args.vision_tower_lr != 0:
                vision_tower_parameters = [
                    n for n, _ in opt_model.named_parameters() if _is_vision_tower_param(n)
                ]
