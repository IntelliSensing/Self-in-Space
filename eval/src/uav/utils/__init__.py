...
class QwenVL(ModelConfig):
    def __init__(
        ...
        if model_size is None or model_size <= 14:
    ...
    def get_prompt_from_question(self, messages: List[Dict]):
        ...
        # 检查是否包含视频或图片内容
        contains_video = messages_contain_video(messages)
...
class InternVL3HF(ModelConfig):
    def __init__(
        ...
        model_size = extract_model_size(model_id)
        if model_size is None or model_size <= 8:
            self.default_engine_args["tensor_parallel_size"] = 1
        elif model_size <= 78:
            self.default_engine_args["tensor_parallel_size"] = 4
            if max_model_len is None:
...
class InternVL(ModelConfig):
    ...
    def __init__(self, model_id: str, max_model_len: int = None, max_tokens: int = None, max_num_frames: int = 32, **kwargs):
...
class Llava(ModelConfig):
    def __init__(self, model_id: str, max_model_len: int = None, max_tokens: int = None, max_num_frames: int = 32, **kwargs):
        ...
        if self.tokenizer.chat_template is None:
            print(f"[INFO] {model_id} has no chat_template, using Vicuna-style prompt format")
        ...
        for msg in messages:
            ...
            for content in msg["content"]:
                if content["type"] == "image":
                    ...
                    images.append(img)
                    new_content.append({"type": "text", "text": "(<image>./</image>)"})
                elif content["type"] == "video":
                    raise NotImplementedError("MiniCPM-V does not support video input in this implementation")
                else:
...
class GenericVL(ModelConfig):
    """Generic VL model class for models whose tokenizer chat template
    natively handles {"type": "image"} content (e.g. Kimi-VL, Step-VL, Ovis)."""
    ...
    def get_prompt_from_question(self, messages: List[Dict]):
        ...
        try:
            ...
            # Fallback for templates that expect content as string (e.g. Ovis2)
            for msg in messages:
                if isinstance(msg["content"], list):
                    content_str = ""
                    for content in msg["content"]:
        ...
        if images:
            return prompt_text, {"multi_modal_data": {"image": images}}
        else:
            return prompt_text
...
def build_model_config(model_id: str, **kwargs) -> ModelConfig:
    if "llava" in model_id.lower():
        ...
        model_config = MiniCPMV(model_id=model_id, **kwargs)
    elif "kimi" in model_id.lower():
        model_config = GenericVL(model_id=model_id, **kwargs)
    elif "step" in model_id.lower() and "vl" in model_id.lower():
        model_config = GenericVL(model_id=model_id, **kwargs)
    elif "ovis" in model_id.lower():
        model_config = GenericVL(model_id=model_id, **kwargs)
    elif "molmo" in model_id.lower():
        model_config = GenericVL(model_id=model_id, **kwargs)
    elif "sail" in model_id.lower() and "vl" in model_id.lower():
        model_config = GenericVL(model_id=model_id, **kwargs)
    else:
